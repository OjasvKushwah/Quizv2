    ...
    try {
        if (period === 'daily') {
            ...
            if (weeklyStats[currentWeek]) {
                stats = { ...stats, ...weeklyStats[currentWeek] };
            }
        } else if (period === 'chapter') {
            loadChapterStats();
            return null; // Chapter stats are handled separately
        }
    } catch (error) {
...
function updateChapterOverview(filteredStats) {
    ...
    const totalAnswered = stats.reduce((sum, chapterStat) => 
        sum + ((chapterStat.correct || 0) + (chapterStat.incorrect || 0)), 0);
